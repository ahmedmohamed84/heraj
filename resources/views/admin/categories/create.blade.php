            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form method="POST" action="{{ route('admin.categories.store') }}">
                        ...
                        <!-- Parent Category -->
                        <div class="mt-4">
                            <label for="parent_id" class="block font-medium text-sm text-gray-700">{{ __('Parent Category (Optional)') }}</label>
                                ...
                                @foreach ($attributes as $attribute)
                                    <label class="flex items-center">
                                        <input type="checkbox" name="attributes[]" value="{{ $attribute->id }}"
                                               ...
                                               @if(is_array(old('attributes')) && in_array($attribute->id, old('attributes'))) checked @endif
                                        >
